...
class MathReasoningApp:
    ...
    def solve_math_problem(self, question, show_reasoning=True, temperature=0.0, max_tokens=1200):
        ...
        if not question.strip():
            return "⚠️ Vui lòng nhập câu hỏi!", "", "0.00s", "Chưa có dữ liệu"
        ...
        try:
            ...
            if tool_count > 0:
                ...
                tool_info = "ℹ️ Không sử dụng công cụ"
            
            return final_answer, reasoning_text, f"⏱️ {execution_time:.2f}s", tool_info
...
def solve_wrapper(question, show_reasoning, temperature, max_tokens):
    ...
    if app is None:
        return (
            "⚠️ Model chưa được tải. Vui lòng khởi động lại ứng dụng.", 
            "", 
            "0.00s", 
            "N/A"
        )
...
def create_interface(model_path="your-username/your-model-name"):
    ...
    with gr.Blocks(css=custom_css, title="AI Math Reasoning", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    clear_btn = gr.Button("🗑️ Xóa", size="lg", scale=1)
        ...
        # Information section
        with gr.Accordion("ℹ️ Hướng dẫn sử dụng & Thông tin", open=False):
            gr.Markdown("""
            ### 📖 Cách sử dụng:
